...
画面上のタイトルは **「神話戦姫」**。`maniwa-tcg` はディレクトリとセーブキー
...
**回復量は必ず、そのワザ自身のダメージ量より小さくすること。** 同値以上にすると、
...
2. 総合力で区切る — **300以上=ウルトラレア / 225以上=スーパーレア / 180以上=レア / それ未満=コモン**
3. **無色は1段下げる** — 全デッキ共通で2枚積みされ、稀少性が低いため

| レアリティ | 表記 | 内部の値 | 種類数 |
|---|---|---|---|
| ウルトラレア | UR | `'ultra'` | 3（EX級） |
| スーパーレア | SR | `'superRare'` | 5 |
| レア | R | `'rare'` | 15 |
| コモン | C | `'common'` | 17 |
...
| レアリティ | 平均使用率 | 平均勝率寄与 |
|---|---|---|
| UR ウルトラレア | 59.1% | +4.1pt |
| SR スーパーレア | 46.1% | +2.9pt |
| R レア | 33.7% | -3.0pt |
| C コモン | 24.1% | -3.4pt |
...
**残りの14.4ptは未解決**。支援カード1枚ずつの実効値を測る仕組み（同じデッキで
