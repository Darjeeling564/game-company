...
草は炎に弱い（`weakness: 'fire'`）が、炎は水に弱いため、**「ほのお」対「くさ」は炎side有利の非対称な組み合わせ**になる。
この状態で先手勝率を測ると、デッキ相性と手番の有利さが混ざって読めなくなる。

したがって12章の**先手勝率（45〜55%）は同一デッキどうしのミラーマッチで測定する**。
デッキ相性は別指標として、対戦組み合わせごとに分けて報告する。
