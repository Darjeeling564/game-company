...
| 順 | 条件 | 結果 |
|---|---|---|
| 1 | 両者が同時に3ポイント以上に達した | 引き分け（`endReason: 'simultaneous'`）※要確認 Q2 |
...
実装前に回答が必要。既定値のまま進めてよければその旨を指示いただきたい。

| # | 論点 | 既定値（回答が無い場合これで進める） |
|---|---|---|
| **Q1** | 先攻1ターン目にエネルギーを供給するか。原作（ポケモンカードゲームポケット）は**供給しない**が、指示は「毎ターン自動で1個供給」だった | **供給する**（指示を優先）。先手勝率が55%を超えた場合の最初の調整候補とする |
| **Q2** | 両者が同時に3ポイントに達した場合（反動ダメージ等） | **引き分け**。「手番プレイヤーの勝ち」にする案もある |
| **Q3** | ベンチをゲーム開始時に何体まで出せるか。初期配置で最大3体まで出せるか、1体ずつ順に出すか | **初期配置で最大3体まで同時に出せる** |
| **Q4** | 「平均試合時間」の定義。実行時間か、人がプレイした場合の想定時間か | **両方出す**（1ターン12秒と仮定） |
| **Q5** | v1 の対戦相手。CPU のみか、同一端末での2人対戦も入れるか | **CPU のみ**（デッキ選択画面で難易度を選ぶ） |
| **Q6** | 題材の扱い。ルールの骨格は原作に倣うが、カード名・キャラクター・イラストは**すべて独自のものを使う**方針でよいか（8章の3枚も独自名） | **独自名で進める** |
