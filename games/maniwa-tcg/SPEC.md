...
**アイテム・行動・絶技には上の区切りをそのまま当ててはいけない。**
支援カードは HP を持たないので `HP` の項が丸ごと落ちる。キャラの HP は 90〜170、
平均でおよそ 110 あり、これがそのまま下駄になっている。キャラの区切りを流用すると
**どんなに強い支援カードでも全部コモンになる**。

そこで支援側は、キャラの区切りから HP の平均ぶん 110 を引いた線を使う。
「同じ強さなら同じレアリティになる」という対応を保つための平行移動である。

| 種別 | 総合力 | UR | SR | R |
|---|---|---|---|---|
| キャラクター | `HP + 効率×3 + 最大威力×0.6` | 300以上 | 225以上 | 180以上 |
| **支援カード** | `効率×3 + 最大威力×0.6` | **190以上** | **115以上** | **60以上** |
...
| SR スーパーレア | 59.9% | +1.3pt |
| R レア | 51.3% | -1.0pt |
| C コモン | 50.3% | -2.3pt |

（10万戦で測定。カード総数が104種、支援カードのレアリティを 8.3 の基準で振り直した時点の値）
