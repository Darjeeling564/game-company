...
| 系列 | 属性 | 内部の値 |
|---|---|---|
| 元素8種 | 炎 / 水 / 森 / 土 / 風 / 鋼 / 雷 / 氷 | `fire` `water` `forest` `earth` `wind` `metal` `thunder` `ice` |
| 対の2種 | 光 / 闇 | `light` `dark` |
| 例外 | 無色 | `colorless` |

`grass` は `forest` に改名する。8元素の並びとして「草」だけ植物名なのは座りが悪い。
既存カードのデータ変更が要るので、承認が取れた時点でまとめて行う。

### 17.2 相性

**元素8種は環にする。各属性は「環の次の2つ」に強い。**

```
炎 → 森 → 土 → 雷 → 鋼 → 氷 → 風 → 水 → 炎（先頭に戻る）
```

| 攻撃側 | 弱点を突ける相手 | 根拠 |
|---|---|---|
| 炎 | 森・土 | 森を焼き、地を焦がす |
| 森 | 土・雷 | 根が地を割り、木が雷を受け止める |
| 土 | 雷・鋼 | 地に雷を逃がし、土砂が鋼を埋める |
| 雷 | 鋼・氷 | 鋼を伝って焼き、氷を砕く |
| 鋼 | 氷・風 | 氷を割り、盾が風を止める |
| 氷 | 風・水 | 風を凍てつかせ、水を凍らせる |
| 風 | 水・炎 | 水を吹き払い、炎を吹き消す |
| 水 | 炎・森 | 炎を消し、森を流す |

環の隣1つだけにすると、不利対面が 1/8 = 12.5% しか起きず、属性を増やした意味が
薄れる。2つにすると 2/8 = 25% になり、今の3すくみ（1/3 = 33%）よりは緩く、
それでも相性を読む意味が残る。

**光と闇は互いにだけ弱点を持つ。** 元素8種とは相性を持たない（どちらからも +0）。
...
現在 `CardDef.weakness` は「弱点となる属性1つ」を持つ。属性が2つに増えるので
`readonly weakness: readonly EnergyType[]` にする。環はデータではなく
...
カード側の `weakness` は表から導出できるので**廃止する**。無色は環の外側なので
弱点を持たず、弱点を突くこともない（現行どおり）。

### 17.5 必要なカード数

10属性を単色デッキで運用するには、1デッキにキャラが14枚（6種×2 + 無色2）要る。
40種を10属性に割ると**1属性4種**しかなく、単色デッキが組めない。

| 前提 | 必要なキャラ種類数 |
|---|---|
| 単色デッキ（属性ごとに6種以上） | **80種**（10属性 × 8） |
| 2色デッキを許す | 60種程度でも可 |

16.6 で決めた目標「キャラ64種」を **80種**に引き上げる必要がある。
...
| # | 論点 | 推奨案 | 理由 |
|---|---|---|---|
| Q13 | 元素8種の相性の広さ | **環の次の2つに強い** | 隣1つだと不利対面が12.5%しか起きず、属性を増やした効果が出ない |
| Q14 | 環の並び | **炎→森→土→雷→鋼→氷→風→水** | 17.2 の根拠どおり、隣接2つが説明できる並びにした |
...
| Q17 | キャラの種類数 | **64種 → 80種**（10属性 × 8） | 単色デッキを組むには1属性6種以上が要る |
