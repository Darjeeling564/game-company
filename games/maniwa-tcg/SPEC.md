...
3.1 のデッキ構築ルール（**20枚ちょうど / 同名2枚まで**）から、**1デッキには最低10種のカードが必要**になる。
つまり上の3枚だけでは合法なデッキを1つも組めず、対戦もシミュレーションも動かせない。
...
| 区分 | モチーフ | 種類数 | 用途 |
|---|---|---|---|
| 無色 | 眷属・霊獣 | 4 | 両デッキ共通のコア |
| 炎 | 火・太陽・破壊の神 | 6 | 「ほのお」デッキ専用 |
| 草 | 大地・豊穣・蛇の神 | 6 | 「くさ」デッキ専用 |

モチーフは世界の神話から**8系統**を取り、**各系統2枚ずつ**均等に配分する。
...
量産時もこの「1系統あたり均等」を保ち、系統が偏らないようにする。

プリセットデッキは2つ。いずれも `専用6種 × 2枚 + 共通4種 × 2枚 = 20枚`。

| デッキ | エネルギー | 構成 |
|---|---|---|
| ほのお | `fire`, `colorless` | 炎6種 + 無色4種 |
| くさ | `grass`, `colorless` | 草6種 + 無色4種 |

### 8.2 弱点は炎と草で相互にする

当初は原作に倣って「草は炎に弱い / 炎は水に弱い」としていたが、v1 には水デッキが無いため
炎side だけが一方的に弱点ボーナスを受け取る形になり、デッキ相性が 78.7 : 21.3 まで偏った。

v1 では**炎と草を相互に弱点とする**（`fire` の弱点は `grass`、`grass` の弱点は `fire`）。
これで両デッキが同じだけ弱点の恩恵を受け、相性は 49.4 : 50.6 に収まる。
水デッキを追加して3すくみを作れる段階になったら、原作どおりの一方向に戻す。
