...
> **状態: 未承認の草案。** 承認前は実装しない（CLAUDE.md 7章）。
> 実験ブランチ `experiment/rarity-summon` に試作と実測がある。
...
| 指標 | 現行 | この案 | 判定 |
|---|---|---|---|
| 先手勝率 | 51.6% | **54.0%** | 基準内（45〜55%）だが上限寄り |
| 引き分け率 | 0.0% | 0.0% | OK |
| 平均ターン数 | 14.7 | 13.7 | 決着が約1ターン速い |
| ポイント決着 | 98.2% | 98.3% | 変化なし |
| ターン上限到達 | 0.0% | 0.0% | OK |
...
先手が先にエネルギーを貯めてリリースできるぶん有利になる。基準（45〜55%）は
満たしているが、現行の 51.6% より 2.4pt 上振れしている。

対処の候補。**どれも未検証なので、実装時に測ってから決める。**

| 案 | 内容 | 懸念 |
|---|---|---|
| a | 何もしない | 基準内ではある。ただし上限に近く、カード追加で外れうる |
| b | 後手の初期手札を1枚増やす | 3.2 の変更。手札上限との兼ね合いを見る必要がある |
| c | 先攻1ターン目はリリースも禁止する | 3.3 の既存制限（攻撃禁止）と揃う。最も小さい変更 |
...
| `game/view.ts` | リリース対象を選ぶ UI（未着手。実験では AI のみで測った） |
