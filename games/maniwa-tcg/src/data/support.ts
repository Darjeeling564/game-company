...
export const ULTIMATES: readonly UltimateCard[] = [
  ...
  /*
   * u017 / u018 は 2026-09-13 の夜間ジョブで追加した。**系統の穴を埋める。**
   * 追加前の絶技16種は クトゥルフ4 / 日本3 / 北欧3 / ギリシア2 / エジプト1 /
   * 中国1 / 中東1 / オリジン1 で、**インドだけが0種**だった。
   * w010 ヴィシュヌ は 2026-09-07 に「インドで初めて最強ワザがコスト3の姫神」として
   * 足したもので、これでその狙いが果たされる。
   */
  {
    ...
    effects: [
      /*
       * ヴィシュヌの最強ワザ「十の顕現」は効率 30.00。ここを 33.33 にして比 1.111 に置く。
       * 攪拌が底から引き上げた不死の霊薬アムリタを回復で表した。
       * 回復25 はダメージ85 より小さい（SPEC 8.2）。
       * インド系統はこれが初の絶技（SPEC 16.5.2 の「絶技を作れない系統」がこれで消える）。
       *
       * **効果は実測で選んだ。** 評価値が同じ100・比も同じ1.111の3案を pool 1万戦で比べると、
       * 勝率寄与が 打点70+gainEnergy +4.0pt / 打点85+回復25 +13.2pt / 打点100のみ +11.5pt
       * と大きく割れた。**絶技は決め手として撃つので、撃ったあとに効く gainEnergy は
       * 評価値30ぶんの働きをしない**（2026-09-13 の夜間ジョブで測定）
       */
  ...
  },
  {
    ...
    effects: [
       ...
       * **絶技で switchOpponent を使うのは初。** 呑み込んで引きずり込む動きを、
       * 相手のバトル場を入れ替える形にした。神具では i021 グングニル が同じ組み合わせ
       */
