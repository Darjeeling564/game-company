 ...
 *
 * 2026-09-25: 48枠のうち9枠を入れ替えた（承認済み）。**6.00pt → 3.75pt**（未見シード）。
 *
 * 神具を2種（i025 九鼎 / i026 黄金の林檎）足した夜に測り直したもの。
 *
 *   山登り140手 ＋ 1手で1〜3枠を同時に入れ替える（6割が1枠・3割が3枠・1割が2枠）
 *   目的関数は3シード（11000/22000/33000）の平均レンジ。1回の評価は 28組×先後2×178戦
 *
 * | | 探索に使った3シード | 探索に使っていない2シード |
 * |---|---|---|
 * | 旧 | 7.45pt | 6.00pt |
 * | 新 | 3.20pt | **3.75pt** |
 *
 * 未見シードで再現するので過学習ではない。2026-09-01 の 5.12pt も下回る。
 * ミラー6000戦で先手勝率 49.3% → 50.0%、平均ターン 14.5 → 14.2 と、どちらも動いていない。
 * 縮んだ分の大半は46〜47%と沈んでいた **ほのお と もり の底上げ**である。
 *
 * **今夜足した i025 / i026 はどちらも選ばれなかった。** 候補には入れてあったが、
 * 140手のあいだ一度も改善にならなかった。つまりこの改善は新カードのおかげではなく、
 * **既存カードの配り直しだけで出た差**である。カードを足したことをきっかけに
 * 測り直したら、前から縮む余地が残っていた、というのが正確なところ。
 *
 * 1枠ずつの山登りは 2026-08-30 に打ち切りになっているので、**複数枠を同時に動かす手を
 * 混ぜること。** 15回の改善のうち4回が複数枠の同時手だった。
 */
