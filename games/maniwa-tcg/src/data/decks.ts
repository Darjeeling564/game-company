 ...
 *
 * 2026-09-01: 48枠のうち21枠を入れ替えた（承認済み）。**16.4pt → 5.1pt**。
 *
 * 山登りは 2026-08-30 に1手で打ち切りになり、「単発の入れ替えではこれ以上縮まない」
 * ところまで来ていた。残っていたのは ほのお 59.6% と つち 42.8% のような
 * デッキ4つぶんの不均衡で、1枠ずつでは埋まらない。そこで**手法を変えた**。
 *
 *   焼きなまし（70手・温度 1.65 → 0.05）＋ 1手で1〜3枠を同時に入れ替える
 *   目的関数は2シードの平均レンジ。検証は 1500戦per組（42,000戦）
 *
 ...
 * | 新 | 5.67pt | **5.12pt** |
 *
 * 未見シードで再現するので過学習ではない。2026-08-22 の最良 6.2pt も下回る。
 * ミラー6000戦で先手勝率 49.5% → 49.4%、平均ターン 14.2 → 14.5 と、
 * どちらも動いていない。
 *
 * この配分は **i015〜i018 / a015〜a018（夜間ジョブが足した新カード）を実際に使う**。
 * 山登りが1枠ずつしか動かせず、新カードを1枚入れる手が単独では改善にならないため
 * 到達できなかった領域である。カードを足したら、また手法ごと見直すこと。
 */
