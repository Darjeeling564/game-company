 ...
 * 判定は「乳首が**露出しているか**」になる。
 * 同じ日に下半身の扱いも明文化した。**陰毛は判定に使わず、性器がはっきり
 * 描かれているかだけを見る。** もともと下半身の条件は書かれていなかった。
...
const COSTUME_NOTE: readonly string[] = [
  '**乳首が露出していないこと。性器がはっきり描かれていないこと。** この2点で決まる',
  '**乳首が描かれていても、覆いで隠れていれば通る**（2026-09-20 に緩和）',
  '**陰毛が描かれていても通る。** 陰毛の有無は判定に使わない（2026-09-20 に明文化）',
