...
/**
 * 衣装の条件（2026-09-10 更新・src/data/art/README.md）。
 *
 * もとは「カップ＋肩紐＋アンダーバンドが繋がった三角ビキニ」という**形**を指定していたが、
 * 覆えているのに形が違うだけの絵まで弾いてしまうため、形の指定を外した。
 * 2026-09-05 に「ニップレス類」「鎖・紐のみ」「布がずれて出ている」の3項目も外し、
 * 2026-09-10 に乳輪も外して、判定は「乳首が描かれていないか」の1点だけになった。
 * 2026-09-20 にさらに緩め、**描かれていても覆いで隠れていれば通る**ようにした。
 * 判定は「乳首が**露出しているか**」になる。
 * 同じ日に下半身の扱いも決めた。いったん「性器がはっきり描かれているものは不可」と
 * したが、**その日のうちに取り消し、下半身は判定に使わないことにした**。
 * 陰毛も性器も、描かれていて通る。
 * 同じ日に 2次創作を明示的に可とし、そのぶん年齢の条件を
 * 「元の作品での描かれ方」まで広げた（src/data/art/README.md）。
 *
 * 生成のたびに手で書き添えていた条件を、ここに移して出力に必ず載るようにする。
 */
const COSTUME_NOTE: readonly string[] = [
  '**乳首が露出していないこと。** 取り込めるかどうかはこの1点で決まる',
  '**乳首が描かれていても、覆いで隠れていれば通る**（2026-09-20 に緩和）',
  '**下半身は判定に使わない。** 陰毛も性器も、描かれていて通る（2026-09-20）',
  '覆いの有無・形・素材・面積は問わない。布・金属・鎧のほか、髪・腕・道具・水面・湯気・影でもよい',
  '薄手・半透明の布、ニップレス類、鎖・紐だけ、覆いが無いものも通る',
  '**乳輪が描かれていても通る。** 乳輪の有無は判定に使わない',
  '**2次創作（実在の作品のキャラクター）も通る。** 個人で遊ぶためのもので、配布はしない',
  '通らないのは次の4つ',
  '  乳首が覆われずに露出している / 人物が成人に見えない / 性行為の描写',
  '  **元の作品で未成年として描かれているキャラクターを、性的に描いたもの**',
  '**性行為の描写とは、複数名が関わるもののこと**（性別は問わない。2026-09-20 に定義）',
  '  **単独の描写は該当しない。** ひとりなら、行為の最中でも通る',
  '  **複数名が写っていること自体は関係ない。** 並んで立っているだけなら通る',
]
...
const KIND_BRIEF: readonly {
  ...
  },
  {
    ...
    title: '絶技',
    note: [
      '対応する姫神が技を放つ瞬間を描く。**同一人物と分かれば衣装は元のカードと違ってよい**',
      '  髪・瞳・角や翼などの特徴を揃える。衣装だけが違うのは通す（2026-09-14 決定）',
