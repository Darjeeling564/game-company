 ...
 *
 * **maniwa-tcg と同じ見た目にそろえている。** 同じ世界の別のゲームなので、
 * 遊ぶ人が2本を行き来したときに同じカードだと分かるほうがよい（SPEC 9.2）。
 *
 * 見た目の役割分担:
 *   背景 = 系統 / 枠 = レアリティ / 属性 = 色丸に白文字 / 文字色 = レアリティ
 */
import type { Attribute, Origin, Rarity } from '../core/types.ts'
 ...
 * 属性の丸の色。中の文字は常に白にする。
 *
 * **属性は戦闘に影響しない**（遊戯王の属性は戦闘計算に関与しない。SPEC 6章）。
 * デッキの色分けと見た目にだけ使う。
 */
export const TYPE_COLOR: Readonly<Record<Attribute, string>> = {
  fire: '#c0392b',
  forest: '#3f7452',
  wind: '#5f9ea0',
  earth: '#8a6a3a',
