...
- 魔法（神具）の発動、罠（道標）のセットと**バトル中の割り込み1段**
...
| **スキル** | デュエルリンクス固有の要素だが、決闘が1本通ってから乗せる。**絶技18種をここに充てる案**（15章） |
| 絶技18種 | 上と同じ。v1 ではカードプールに入れない |
| フィールド魔法ゾーン | 使うカードが無い |
| 永続魔法・装備魔法・カウンター罠 | 効果の種類を増やす前にルールを固める |
| チェーン2段以上 | 11章。v1 は1段 |
...
export interface SpellDef {
  readonly id: CardId          // i001 など
  readonly name: string
  readonly ruby?: string
  readonly kind: 'spell'       // 神具
  readonly flavor: string
...
**52種すべての `effects` を書き直す。** 名前・説明文・イラストは変えない。
...
| 項目 | 備考 |
|---|---|
| **スキル** | デュエルリンクス固有の要素。**絶技18種をここに充てる**。8つのデッキに性格を与えられ、`maniwa-tcg` で絶技が抱えていた「対応する姫神がいないと死に札」問題も起きない |
| チェーン2段以上 | 11章。`pendingAttack` を解決スタックに置き換える |
| 永続魔法・装備魔法・カウンター罠 | 効果の種類を増やす |
